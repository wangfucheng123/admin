      ...
      <form name="edtForm" id="edtForm" action="" method="post">
        ...
        <ul class="forminfo" id="ul">
         ...
         <!--  <li ><label>出生日期</label><input name="birthday" id="birthday" type="text" class="Wdate" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',readOnly:true,maxDate:'%y-%M-%d'})" /></li> 
          ...
          <li><label>&nbsp;</label><input name="" onclick="submitForm();" type="button" class="btn" value="确认注册" /></li>
        </ul>
      </form>
    </div>
...
function loadTable()
{
    ...
    console.log(data)
		...
		for(var i=0;i<content.length;i++)
		{
			...
			html=html+'<tr><td>'+returnVal(content[i].realname)+'</td><td>'+returnVal(content[i].phone)+'</td><td>'+new function(){if(returnVal(content[i].birthday)=='') return new String('');else return new String(new Date(returnVal(content[i].birthday)).pattern("yyyy-MM-dd"));}+'</td><td>'+returnVal(content[i].email)+
			'</td><td><a href="#" onclick="openDetail('+content[i].id+
			');">修改</a>&nbsp;<a onclick="removeT('+content[i].id+
			');" href="#">删除</a>&nbsp;<a onclick="openRole('+content[i].id+
      ',1);" href="#">角色添加</a>&nbsp;<a onclick="openRole('+content[i].id+
			',2);" href="#">角色修改</a></tr>';
		}
