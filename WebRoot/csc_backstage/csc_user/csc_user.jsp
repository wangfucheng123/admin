...
function loadTable()
{
	var data=JSON.parse(getData('cscUser/userList?1=1',pageNum));
	if(data!=null)
	{
		...
		for(var i=0;i<content.length;i++)
		{
			...
			html=html+'<tr><td>'+returnVal(content[i].name)+'</td><td>'+returnVal(content[i].userId)+'</td><td>'+returnVal(content[i].typeName)+'</td><td>'+
			...
			');">修改</a>&nbsp;<a onclick="comm(\'cscUser/deleCscUser?id='+content[i].id+
			'\');" href="#">删除</a>&nbsp;<a onclick="openRole('+content[i].userId+
			');" href="#">权限管理</a></td></tr>';
		}
...
{
	window.open('cscUserRole.jsp?userId='+userId);
}
</script>