		...
		<div id="dialog" class="Popup" style="display: none; width: 200;">
			...
			<form name="edtForm" id="edtForm" action="" method="post">
				...
				<ul class="forminfo" id="ul">
					...
					<li><label>分类</label>
						<div class="vocation">
							<select name="sort" id="sort" class="dfinput">
								<option value="数金用户管理员">数金用户管理员</option>
								<option value="数金运营管理员">数金运营管理员</option>
								<option value="数金订单管理员">数金订单管理员</option>
							</select>
						</div></li>
					<li><label>手机号码</label><input name="phone" id="phone"
						type="text" onblur="checkPhone(this);" class="dfinput" /></li>
					<li><label>真实姓名</label><input name="realname" id="realname"
						"text" class="dfinput" /></li>
					<li><label>邮箱地址</label><input name="email" id="email"
						onblur="checkEmail(this);" type="text" class="dfinput" /></li>
...
<script type="text/javascript">
	...
	function loadTable() {
		...
		$.ajax({
			type : "POST",
			dataType : "json",
			url : "${pageContext.request.contextPath}/view/cscrole/getRoleList",
			success : function(data) {
				var content = $.parseJSON(data);
				for (var i = 0; i < content.length; i++) {
					html = html + '<tr id="' + content[i].roleId
							+ '" onclick="loadPrivileges(' + content[i].roleId
							+ '); "><td>' + content[i].roleName
							+ '</td><td><a href="#" onclick="openDetail('
							+ content[i].id
							+ ');">修改</a>&nbsp;<a onclick="removeT('
							+ content[i].id
							+ ');" href="#">删除</a>&nbsp;<a onclick="resetPW('
							+ content[i].id + ');" href="#">重置密码</a></td></tr>'
	...
	function loadPrivileges(userid) {
		...
		$.ajax({
					type : "POST",
					dataType : "html",
					url : "${pageContext.request.contextPath}/authority/get_authoritys.do?userid="
							+ userid,
					success : function(data) {
						...
						if (data != null) {
							for (var json = 0; json < data.length; json++) {
								...
								var tr = "";
								var tr = '<tr id="tr'+data[json].authorityId+'"><td><select disabled="disabled" style="width:200" name="name'+len+'" id="id'+data[json].authorityId+'"></select><td><a href="#" onclick="save_authority('
										+ data[json].authorityId
										+ ',\'\');">保存</a>&nbsp;<a href="#" onclick="enable('
										+ data[json].authorityId
								...
								$.ajax({
											...
											success : function(r) {
												...
												for (var num = 0; num < r.length; num++) {
													if (r[num].id == id) {
														option = option
																+ '<option value="'+r[num].id+'" selected="selected">'
																+ r[num].name
																+ '</option>';
													} else {
														option = option
																+ '<option value="'+r[num].id+'">'
																+ r[num].name
																+ '</option>';
													}
												}
	...
	function addTr() {
		...
		var tr = '<tr id="tr'+i+'"><td><select  name="name'+i+'" style="width:200" id="id'+i+'"></select><td><a href="#" onclick="save_authority(\''
				+ "id"
				+ i
				+ '\');">保存</a>&nbsp;<a onclick="removeElement('
				+ i
				+ ')" href="#">删除</a></td></tr>';
		var option = '';
		$.ajax({
					...
					success : function(data) {
						...
						for (var json = 0; json < r.length; json++) {
							option = option + '<option value="'+r[json].id+'">'
									+ r[json].name + '</option>';
						}
						$('#id' + i).append(option);
					}
